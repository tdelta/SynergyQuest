/*
 * This is code directly extracted from the GrShell of GrGen.NET and the YCompClient Example API code
 * TODO: This code must be extracted into a library before distributing, otherwise Unity is in conflict with the LGPL
 * 
...
namespace de.unika.ipd.grGen.grShell
{
    ...
    public class YCompClient
    {
        ...
        public static YCompClient CreateYCompClient(string ycompPath, INamedGraph graph, String layout, DumpInfo dumpInfo)
        {
            ...
            try
            {
                Process.Start(ycompPath, "--nomaximize -p " + ycompPort);
            }
            catch(Exception e)
            {
            ...
            try
            {
                return new YCompClient(graph, layout, 20000, ycompPort, dumpInfo, new ElementRealizers(), new Dictionary<string, IObject>());
            }
            catch(Exception ex)
            {
